...
The fitting process begins at block A, where nothing has happened yet. When the client sends a fit request to the endpoint `/v1/chimera/aggregation/fit`, at block B, the Master receives the request and broadcasts it to all workers via the endpoint `/v1/chimera/model/fit`, as shown in block C.

Then, concurrently, the Master waits for either a response, a timeout, or the maximum number of retries from each worker—this is represented by block D. If a worker times out or reaches the retry limit, it is considered failed. In the conditional block E, if at least one worker responds successfully, the flow proceeds to block G. There, the Master receives an "ok" message with status code 200 from the responsive workers, indicating that their local models have been successfully fitted using their training data. Finally, in block H, the Master sends a JSON-formatted response to the client via the endpoint `/v1/chimera/aggregation/fit`, containing an "ok" message to confirm that the distributed model is now fitted. The flow then ends at block I.

If all workers fail in block E, the flow moves to block F, where the Master returns a JSON-formatted error message to the client via the endpoint `/v1/chimera/aggregation/fit`, indicating the error. Finally, the flow ends at block I.
...
The prediction process begins at block A, where nothing has happened yet. When the client sends a prediction request to the endpoint `/v1/chimera/aggregation/predict`, at block B, the Master receives the request and broadcasts it to all workers via the endpoint `/v1/chimera/model/predict`, as shown in block C.

Then, concurrently, the Master waits for either a response, a timeout, or the maximum number of retries from each worker—this is represented by block D. If a worker times out or reaches the retry limit, it is considered failed. In the conditional block E, if at least one worker has responded successfully, the flow proceeds to block G. There, the Master receives results from the responsive workers—status code 200 responses containing predictions from each local model. In block H, the Master aggregates these results by computing the arithmetic mean, and in block I, it returns a JSON-formatted response to the client via the endpoint `/v1/chimera/aggregation/predict`, containing the final predictions. The flow then ends at block J.

If all workers fail in block E, the flow goes to block F, where the Master returns a JSON-formatted error message to the client via the endpoint `/v1/chimera/aggregation/predict`, indicating the issue. Finally, the flow ends at block J.
...
The fitting process begins at block A, where nothing has happened yet. When the client sends a fit request to the endpoint `/v1/chimera/parameter-server/fit`, at block B, the Master receives this request. It then performs an initial fit on its model using sample data collected from the workers, as represented in block C. This sample data is requested from any of the workers via the endpoint `/v1/chimera/sgd/request-data-sample`. The Master sends requests sequentially to the workers, and the first one to respond with a status code 200 provides the sample data. This data consists of a few rows from the worker's local training dataset. In regression, the Master learns the data columns; in classification, it also learns all the classes present in the dataset.

After the initial fit, the Master broadcasts a fit block to each worker via the endpoint `/v1/chimera/sgd/fit-step`, sending each one the current model's coefficients and intercept—this occurs in block D. From there, the Master concurrently waits for a response, a timeout, or the maximum number of retries from each worker. If all workers fail (block F)—that is, if every worker times out or exceeds the retry limit—the flow proceeds to block G and returns an error message through the endpoint `/v1/chimera/sgd/fit`. The process ends at block M.

If at least one worker responds, the Master receives results consisting of gradients for both the coefficients and the intercept. In block I, the Master aggregates these gradients by computing their arithmetic mean. Then, in block J, it checks for convergence or whether the maximum number of iterations has been reached. If either condition is met, the flow proceeds to block L, returns an "ok" JSON-formatted response via the endpoint `/v1/chimera/sgd/fit`, and concludes at block M. However, if the model has not converged and the maximum iterations have not been reached, the flow moves to block K, where the Master updates its model’s coefficients and intercept using the SGD formula. The loop continues from block D until either condition F or J is satisfied.
...
The prediction process begins at block A, where nothing has happened yet. When the client sends a prediction request to the endpoint `/v1/chimera/parameter-server/predict`, at block B, the Master receives the request. Then, in block C, if the model on the Master has been fitted—that is, if the endpoint `/v1/chimera/parameter-server/fit` was previously called and returned a status code 200—the Master returns the predictions to the client as a JSON-formatted response via the same prediction endpoint. Finally, the flow ends at block F.

If, in block C, the model has not been fitted, an error message is returned to the client through the endpoint `/v1/chimera/parameter-server/predict`. The flow then ends at block F, as before.
